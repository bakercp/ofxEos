...
void ofApp::setup()
{
    ...
    std::string contourfile = ofToDataPath("sfm_model_contours.json", true);
    // file with model's precomputed edge topology
    std::string edgetopologyfile = ofToDataPath("sfm_3448_edge_topology.json", true);
    // file with blendshapes
    std::string blendshapesfile = ofToDataPath("expression_blendshapes_3448.bin", true);
    ...
    {
        std::cout << "Error loading the Morphable Model: " << e.what() << endl;
        ofExit();
    }
    ...
    const std::vector<eos::morphablemodel::Blendshape> blendshapes = eos::morphablemodel::load_blendshapes(blendshapesfile);
    
    // These two are used to fit the front-facing contour to the ibug contour landmarks:
    const eos::fitting::ModelContour model_contour = contourfile.empty() ? eos::fitting::ModelContour() : eos::fitting::ModelContour::load(contourfile);
    const eos::fitting::ContourLandmarks ibug_contour = eos::fitting::ContourLandmarks::load(mappingsfile);
    
    // The edge topology is used to speed up computation of the occluding face contour fitting:
    const eos::morphablemodel::EdgeTopology edge_topology = eos::morphablemodel::load_edge_topology(edgetopologyfile);
